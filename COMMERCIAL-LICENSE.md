# Commercial Licensing

Phaselith is available under `AGPL-3.0-or-later` for open-source use.

If you want to use Phaselith without AGPL obligations, a separate commercial license is required.

Typical commercial-license scenarios include:

1. Embedding Phaselith into proprietary software.
2. Redistributing Phaselith in closed-source desktop, plugin, device, or OEM products.
3. Offering Phaselith as part of a paid service without releasing the corresponding source code required by AGPL.
4. Shipping modified internal forks that you do not want to distribute under AGPL terms.

Commercial licensing is intended for:

- proprietary desktop applications
- audio products and OEM integrations
- SDK and white-label deals
- enterprise or platform deployment

## What a Commercial License Can Cover

Depending on the agreement, a commercial license may include:

1. Proprietary redistribution rights
2. Closed-source modification rights
3. Internal commercial deployment rights
4. OEM / platform integration rights
5. Priority support or custom engineering terms

## Contact

Commercial use requires a paid license and separate commercial discussion with the project owner.

Contact Howard Chen (chenhoward0401@gmail.com) to start that discussion.

## Important Note

This file is only a high-level commercial-license notice.

It is not itself the commercial contract.
