...
CIRRUS 是一個即時感知音訊修復與呈現引擎，旨在讓受損、有損壓縮、空間塌縮或刺耳的播放音訊聽起來更聚焦、更清晰，並在聽者面前產生更真實的物理定位感。

本倉庫包含 CIRRUS 核心、瀏覽器執行環境、Windows APO 執行環境，以及將 DSP 引擎轉化為實際產品所需的控制層程式碼。
...
這就是為什麼 CIRRUS 能在不像傳統 EQ 的情況下，聽起來像是減少了混濁、恢復了前方聚焦，或改善了結像鎖定。
...
注意：crate 名稱內部仍使用歷史遺留的 `asce-*` 前綴。此處記錄的產品和架構身份為 `CIRRUS`。

## 狀態

> **早期預覽** — CIRRUS 正在積極開發中。核心演算法已穩定且產出強勁的聽感結果，但 API、設定選項和執行環境架構可能在未通知的情況下變更。
...
商業授權適用於希望在無 AGPL 義務下使用、嵌入或重新發布 CIRRUS 的團隊。
