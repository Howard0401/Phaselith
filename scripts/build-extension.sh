...
cargo build -p asce-wasm-bridge --target wasm32-unknown-unknown --release

echo "Copying WASM to extension..."
cp "$ROOT_DIR/target/wasm32-unknown-unknown/release/asce_wasm_bridge.wasm" "$EXT_DIR/asce_wasm_bridge.wasm"

# Optional: optimize with wasm-opt if available
if command -v wasm-opt &> /dev/null; then
  echo "Optimizing WASM with wasm-opt..."
  wasm-opt -Oz "$EXT_DIR/asce_wasm_bridge.wasm" -o "$EXT_DIR/asce_wasm_bridge.wasm"
fi
